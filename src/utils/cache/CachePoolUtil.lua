...
function CachePoolUtil.addArmatureFileAsync(name,callback,suffix)
	table.insert(CachePoolUtil.nameList, {["name"] = name, ["suffix"] = suffix})
	local pvr = suffix and "res/"..name .. suffix or "res/"..name .. ".pvr.ccz"
	local plist = "res/"..name .. ".plist"
	local xml = "res/"..name .. ".xml"
...
function CachePoolUtil.removeAllArmatureFile()
	for i = 1, #CachePoolUtil.nameList do
		qy.tank.utils.cache.CachePoolUtil.removeArmatureFile(CachePoolUtil.nameList[i]["name"], CachePoolUtil.nameList[i]["suffix"])
