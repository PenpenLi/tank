...
function BaseView:onTableViewClick(name, callback, extendObj,direcion)
    assert(callback, "callback 不能为空")
    ...
    local flag = true
    local beginTouchPoint = {x=0,y = 0}
    local MoveTouchPoint = {x=0,y = 0}
    local offset = 5;
    ...
    view:addTouchEventListener(function(sender, eventType)
         ...
         if eventType == ccui.TouchEventType.ended then
            if flag then
                ...
                qy.QYPlaySound.playEffect("sound/dianji.mp3")
        ...
        if eventType == ccui.TouchEventType.moved then
            ...
            if direction == 1 then
                if math.abs(MoveTouchPoint.y - beginTouchPoint.y) > offset then
                    flag = false
                end
            end
            if direction == 2 then
                if math.abs(MoveTouchPoint.x - beginTouchPoint.x) > offset then
                flag = false
                end
            end    
        end
    ...
    if tolua.cast(view,"cc.Node") then
        ...
        view:addTouchEventListener(function(sender, eventType)
            ...
            if eventType == ccui.TouchEventType.began then
                ...
                if extendObj.isScale then
                    view:setScaleX(scaleX * 0.90)
                    view:setScaleY(scaleY * 0.90)
                end
            ...
            if eventType == ccui.TouchEventType.moved then
                MoveTouchPoint = sender:convertToWorldSpace(cc.p(sender:getPositionX(),sender:getPositionY()))
                if math.abs(MoveTouchPoint.y - beginTouchPoint.y + MoveTouchPoint.x - beginTouchPoint.x) > offset then
                    flag = false
                end
            end
