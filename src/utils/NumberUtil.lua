...
--取一个整数的后几位 一个整数 整数的位数 保留后几位 123456 6 2  结果56
function NumberUtil.getNumber(number,num,jiwei)
    local length = jiwei+1
    local leftNum = number
    local yushu = 0
     local ncifang = 0
    for i=num,length,-1 do
        ncifang = math.pow(10, (i - 1))
        yushu = math.mod(leftNum, ncifang)
        leftNum = yushu
    end
    return leftNum
end
