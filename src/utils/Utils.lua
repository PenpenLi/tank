...
function Utils.getPosByTileNum(num, width_size, height_size, width_height)
    local _width  = width_size  or qy.DEFAULT_WIDTH_SIZE
    local _height = height_size or qy.DEFAULT_HEIGHT_SIZE
    local _width_height = width_height or qy.DEFAULT_WIDTH_HEIGHT



    return cc.p(((tonumber(num) - 1) % _width + 1) * qy.DEFAULT_WIDTH_HEIGHT, (_height - math.floor(tonumber(num) / _width)) * qy.DEFAULT_WIDTH_HEIGHT)
